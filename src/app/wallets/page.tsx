            ...
            {filteredWallets.map((wallet) => (
              <button
                ...
                className={`w-full p-4 text-left hover:bg-gray-50 transition-colors ${
                  selectedWallet.id === wallet.id ? 'bg-blue-50 hover:bg-blue-50' : ''
                }`}
              >
                <div className="flex items-start justify-between mb-2">
                  <span className="font-mono text-sm font-medium text-gray-900">{wallet.id}</span>
                  <span
                    className={`text-lg font-semibold ${
                      wallet.accuracy >= 70 ? 'text-green-600' : 'text-yellow-600'
                    }`}
                  >
                    {wallet.accuracy}%
                  </span>
                </div>
                <div className="text-xs text-gray-600 mb-1">{wallet.specialization}</div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-2xl font-semibold text-gray-900 mb-2">{selectedWallet.id}</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-2xl font-semibold text-green-600">{selectedWallet.accuracy}%</div>
              </div>
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 text-gray-600 mb-1">
                ...
                <div className="text-2xl font-semibold text-gray-900">{selectedWallet.totalTrades}</div>
              </div>
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 text-gray-600 mb-1">
                ...
                <div className="text-2xl font-semibold text-gray-900">{selectedWallet.avgLeadTime}h</div>
              </div>
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 text-gray-600 mb-1">
              ...
              {selectedWallet.recentActivity.map((activity, index) => (
                ...
                >
                  <div className="flex-1">
                    <div className="font-medium text-gray-900 mb-1">{activity.marketTitle}</div>
                    <div className="text-sm text-gray-600">
                      {formatDistanceToNow(activity.timestamp, { addSuffix: true })}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-semibold text-blue-600">{activity.position}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
            <div className="mb-3">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Get alerts for wallet activity</h3>
              <p className="text-sm text-gray-600 mb-4">
                Upgrade to Pro to follow wallets and receive instant notifications when they enter new positions
              </p>
