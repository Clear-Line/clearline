...
export default function Navbar({ isLoggedIn = false, onAuthAction }: NavbarProps) {
  ...
  return (
    <header className="w-full border-b border-gray-200 bg-white sticky top-0 z-50">
      <nav className="max-w-screen-xl mx-auto px-6 h-16 flex items-center justify-between gap-8">
        ...
        {/* Desktop Nav Items — flex-1 pushes actions to the right */}
        ...
        {/* Right Side — shrink-0 keeps buttons from squishing */}
        <div className="hidden md:flex items-center gap-3 shrink-0">
          <button
            onClick={onAuthAction}
            className="text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors px-4 py-2 rounded-lg hover:bg-gray-50 whitespace-nowrap"
          >
            {isLoggedIn ? "Sign out" : "Sign in"}
          </button>
          {!isLoggedIn && (
            <button className="text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-colors px-5 py-2 rounded-full whitespace-nowrap">
              Upgrade to Pro
            </button>
          )}
        </div>
      ...
      {mobileOpen && (
        <div className="md:hidden px-4 pb-4 pt-2 space-y-1 border-t border-gray-100">
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
            ...
            {!isLoggedIn && (
              <button className="flex-1 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 py-2 rounded-full">
                Upgrade to Pro
              </button>
            )}
          </div>
