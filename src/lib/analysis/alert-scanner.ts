...
async function scanVolumeExplosions(startTime: number): Promise<Alert[]> {
...
async function scanSmartMoneyEntries(startTime: number): Promise<Alert[]> {
  if (Date.now() - startTime > TIME_BUDGET_MS) return [];
...
async function scanRangeBreakouts(startTime: number): Promise<Alert[]> {
  if (Date.now() - startTime > TIME_BUDGET_MS) return [];
...
async function scanManipulationWarnings(startTime: number): Promise<Alert[]> {
  if (Date.now() - startTime > TIME_BUDGET_MS) return [];
...
export async function scanForAlerts(): Promise<{
  ...
  const [volumeAlerts, smartMoneyAlerts, breakoutAlerts, manipAlerts] = await Promise.all([
    scanVolumeExplosions(startTime).catch(() => [] as Alert[]),
    scanSmartMoneyEntries(startTime).catch(() => [] as Alert[]),
    scanRangeBreakouts(startTime).catch(() => [] as Alert[]),
    scanManipulationWarnings(startTime).catch(() => [] as Alert[]),
  ]);
